...
public class PigService {
	...
	db에 가져온 sql에 추가적으로 수정을 서비스단계에서 진행한다음 컨트롤러에 전달하거나 전달받기
	*/
	...
	public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		...
		if(!폴더경로확인.exists()) {
			폴더경로확인.mkdirs(); // mkdir = 폴더 한개, mkdirs = 폴더 여러개
			// mkdir 써도 되긴 하지만 혹시모를 문제를 최소하하기 위해 mkdirs를 사용
		}
		...
		try {
			...
			//pig.setPig_image_path(이미지저장폴더); // String 값이기 때문에 File 자료형이 올 수 없음
			pig.setPig_image_path(DB에파일경로및파일명전달);
			
			pigMapper.uploadPig(pig);
			
			System.out.println("Mapper에 Service로 pig insert전달 성공"); // -> 나중에 log로 변경
		} catch (IOException e) {
