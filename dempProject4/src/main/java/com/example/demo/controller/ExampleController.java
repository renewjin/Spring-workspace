...
public class ExampleController {
	...
	public String ex1(HttpServletRequest req, Model model) {
		 ...
		 * - @SessionAttributes와 함꼐 사용할 경우 session scope 반환
		 * [기본 사용법]
		 * Model.addAttribute("key"               ,    value);
		 * Model.addAttribute("html에 전달가능한 이름" , 전달할 갑을 작성);
		 * */
		...
		mem.setMemberNo(1);
		mem.setMemberName("동그라미씨");
		mem.setMemberAge(25);
		
		model.addAttribute("mem", mem);
		
		/*
		 * model.addAttribute("a", 별칭이 아래와 똑같지 않아도 됨);
		...
		return "example/ex1"; // templates/example/ex1.html 파일 바라보는것
	}
	...
	public String ex4(Model model) {
		...
		//123 둘리 33
		MemberDTO mem = new MemberDTO(123, "둘리", 33);
		model.addAttribute("mem", mem);
